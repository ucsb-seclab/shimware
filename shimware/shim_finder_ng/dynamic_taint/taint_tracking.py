...
class TaintTracker(ExplorationTechnique):
    ...
    def __init__(self,
        ...
        :param should_follow_call: callback that specify the policy to follow a call
        """        
        ...
        self._N = n_iter_loop
        self._should_follow_call = should_follow_call
        self._stop = Event()
    ...
    def _should_follow_call_default(self, state):
        """
            ...
            if self._use_smart_concretization:
                if self._taint_deref_values:
                    s.inspect.b(
                        ...
                        angr.BP_AFTER,action=self._addr_concrete_after)
    ...
    def step(self, simgr, *kargs, **kwargs):
        ...
        for s in simgr.active:
            ...
            if s.history.jumpkind == "Ijk_Call":
                should_follow_call = self._should_follow_call if self._should_follow_call else self._should_follow_call_default
                if should_follow_call(s):
                    # follow the call
